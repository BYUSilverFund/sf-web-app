...
export async function getBenchmarkSummary(
    ...
    try {
        const response = await fetch(process.env.NEXT_PUBLIC_FASTAPI_URL + "benchmark/summary", {
            method: "POST",
        ...
        const benchmarkSummaryResponse: BenchmarkSummaryResponse = await response.json();
