...
export async function getAllHoldingsSummary(
    ...
    try {
        const response = await fetch(process.env.NEXT_PUBLIC_FASTAPI_URL + "all-holdings/summary", {
            method: "POST",
        ...
        const allFundsSummaryResponse: AllHoldingsSummaryResponse = await response.json();
